...
/**
 * Performance test for concurrent batch processing that mirrors the structure of
 * {@code performanceTestSDKMultiThreaded}: each thread sends its own sequence of
 * batch files independently using
 * {@link CnpBatchFileRequest#sendToCnpSFTPConcurrent()}.
 *
 ...
 *     --tests "io.github.vantiv.sdk.performanceTestBatchConcurrent"
 * </pre>
 * Or as a standalone main class:
 * <pre>
 *   java -cp &lt;classpath&gt; io.github.vantiv.sdk.performanceTestBatchConcurrent
 * </pre>
 */
public class performanceTestBatchConcurrent {

    private List<BatchPerformanceThread> testPool = new ArrayList<BatchPerformanceThread>();

    static String merchantId = "07103229";

    public static void main(String[] args) throws Exception {
        performanceTestBatchConcurrent test = new performanceTestBatchConcurrent();
        test.performTest();
    }

    public performanceTestBatchConcurrent() {
        for (int x = 0; x < 50; x++) {
            BatchPerformanceThread t = new BatchPerformanceThread(1000 + x);
            testPool.add(t);
        }
    }

    public void performTest() {
        for (BatchPerformanceThread t : testPool) {
            t.start();
        }
    ...
    class BatchPerformanceThread extends Thread {
        ...
        private static final int TRANSACTIONS_PER_FILE = 50;

        private Properties config;

        public BatchPerformanceThread(long idNumber) {
            threadId = idNumber;
            try {
                ...
                config.setProperty("batchRequestFolder",
                        tmpBase + File.separator + "cnpPerfBatchConcurrentRequests");
                config.setProperty("batchResponseFolder",
                        tmpBase + File.separator + "cnpPerfBatchConcurrentResponses");
        ...
        /**
         * Sends one batch file and processes the response.
         *
        ...
        private long doCycle(int cycleIndex) {
            requestCount++;
            String fileName = "cnpSdk-perfBatchConcurrent-t" + threadId
                    + "-c" + cycleIndex
            ...
            try {
                ...
                for (int i = 0; i < TRANSACTIONS_PER_FILE; i++) {
                    Sale sale = new Sale();
                    sale.setReportGroup("perfTestGroup");
                ...
                CnpBatchFileResponse fileResponse = batchFile.sendToCnpSFTPConcurrent();

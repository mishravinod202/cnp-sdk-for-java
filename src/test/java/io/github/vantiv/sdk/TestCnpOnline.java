...
public class TestCnpOnline {
	...
	public void testCnpOnlineException() throws Exception {
		...
		if ("true".equalsIgnoreCase(config.getProperty("oltpEncryptionPayload"))) {
			when(
					mockedCommunication
							.requestToServer(
									matches("(?s).*?<cnpOnlineRequest.*?<encryptedPayload>(.*?)</encryptedPayload>.*?\n"),
									any(Properties.class)))
					.thenReturn(
	...
	public void testQueryTransactionResponse_notFound() {
		...
		if ("true".equalsIgnoreCase(config.getProperty("oltpEncryptionPayload"))) {
			...
			when(mockedCommunication.requestToServer(matches(".*?<cnpOnlineRequest.*?<queryTransaction.*id=\"1234\".*?customerId=\"customerId\".*?<origId>org1</origId>.*?<origActionType>A</origActionType>.*?"),
	...
	public void testQueryTransactionResponse_transactionFound() {
		...
		if ("true".equalsIgnoreCase(config.getProperty("oltpEncryptionPayload"))) {
			...
			when(mockedCommunication.requestToServer(matches(".*?<cnpOnlineRequest.*?<queryTransaction.*id=\"findId\".*?customerId=\"customerId\".*?<origId>org1</origId>.*?<origActionType>A</origActionType>.*?"),
